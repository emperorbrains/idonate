    ...
    <div class='border-bottom row no-gutters'>
      <div class='col-md border-right'>
        <div class='p-4 p-md-5'>
          ...
          <event-poster-component class='mb-4' v-model="event.posterUrl" :mode="event.posterMode" @mode="eventMode"></event-poster-component>
      ...
      <div class='col-md'>
        <div class='p-4 p-md-5'>
          ...
          <event-donation-component class='mb-4' v-model='event.embedId' :embedOptions="embedOptions"></event-donation-component>
        </div>
      </div>
    </div>
...
export default class EditAdditionalEventDetails extends BaseEventVue {
  ...
  private embedOptions: ISelect[] = [];


  private mounted() {
    this.embedOptions = this.organizationEmbeds;

    this.api.getEvent(this.$route.params.eventId)
      .then((res) => {
        const eventUpdate = this.handleSuccess(res, 'data.event', this.ERROR_UPDATE_PATH);
        this.event = Object.assign(new EventDTO(), EventDTO.toAdditional(eventUpdate));
        this.storeEvent(eventUpdate);
      }).catch(this.handleFailure);
  }

  private eventMode(mode) {
    this.event.posterMode = mode;
  }
  private updateMoreDetails() {
    ...
    const mutation: any = {
        ...
        summary: evt.summary,
        terms: evt.terms,
        embedId: TypeUtils.isUUID(evt.embedId) ? evt.embedId : null,
        posterMode: evt.posterMode,
    };
    if (evt.posterMode === 'HIDDEN' || evt.posterMode === 'VIDEO_EMBED') {
      mutation.posterUrl = evt.posterUrl;
    }
    ...
    Promise.all(promises).then((validations) => {
      ...
      this.api.updateEvent(evt.id, mutation)
        .then((res) => {
          this.handleSuccess(res, '', 'data.event.update.errors');
          this.updateEventStepAndProceed(evt.id, this.PAGE, 'edit-event-design');
        }).catch(this.handleFailure);
    });
  }
}
