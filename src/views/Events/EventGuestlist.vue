...
export default class EventGuestlist extends BaseEventVue {
  ...
  private dropdownFilter: any = { label: 'Ticket#', value: 'id' };
  private notes = '';
  private dropdown = 'S';
  ...
  private ticketIDList: any[] = [
    ...
    { label: 'Ticket Type', value: 'ticketName' },
  ];

  private guestList: any[] = [];
  private showGuestsPerPageList = EventConstants.ITEMS_PER_PAGE;
  ...
  private created() {
    ...
    this.api.getGuestlist(this.$route.params.eventId)
      .then((res) => {
        const guests = this.handleSuccess(res, 'data.event');
        this.guestList = TicketInstanceDTO.fromModel(guests.ticketInstances);
        this.storeEvent(guests);
