      ...
      <div class='p-2'>
        ...
        <div class='mb-3'>
          ...
          <select-component
            ...
            v-model='value.ticketDefinitionId'
            required
          ></select-component>
        </div>
...
export default class NewComplementaryTicketModal extends BaseEventVue {
  public $refs!: {
    modal: any;
  };
  ...
  private submitForm() {
    ...
    Promise.all(promises).then((validations) => {
      ...
      this.api.createComplimentaryTicket(this.eventId, TicketInstanceDTO.toCompTicketModel(this.value))
        .then((res) => {
          this.updateEventDontProceed(res, 'data.event.createCompTicket.errors');
  ...
  private created() {
    ...
    .then((res) => {
      ...
      this.ticketDefinitions = TicketDefinitionDTO.toList(data, true);
    });
  }
