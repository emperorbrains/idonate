<template>
  <div>
    <h5 class="font-weight-semibold">{{ (index + 1) + '. ' + question.title }}<span class="ml-2" v-if="question.isResponseRequired">*</span></h5>

    <div v-if="question.type === 'fill-in-the-blank'">
      <input type="text" class="form-control rounded-pill" :required="question.isResponseRequired">
    </div>
    ...
    <div v-else-if="question.type === 'MULTIPLE_CHOICE'">
      <div
        ...
        :key="oIndex"
      >
        <input type="radio" :id="'question-' + index + '-option-' + oIndex" :name="'question-' + index" class="custom-control-input" :value="option.value" :required="question.isResponseRequired">
    ...
    <div v-else-if="question.type === 'MULTIPLE_CHOICE_DROPDOWN'">
      <select class="custom-select" :required="question.isResponseRequired">
        <option
          v-for="(option, oIndex) in question.options"
          :index="oIndex"
          :key="oIndex"
    ...
    <div v-else-if="question.type === 'textarea'">
      <textarea cols="30" rows="4" class="form-control" :required="question.isResponseRequired"></textarea>
    </div>
  </div>
</template>
