  ...
  <div>
    ...
    >
      <template slot="attendee" slot-scope="data">
        ...
        <div class="text-success" style="font-size: 10px;">{{data.item.ticketId}}</div>
      </template>
...
@Component
export default class AttendeesListComponent extends BaseVue {
  ...
  private loadAttendees() {
    this.api.getOrderAttendees(this.orderId)
      .then((res) => {
        ...
        this.attendees = TicketInstanceDTO.fromModelToAttendeeList(comps);
  ...
  private updateAttendeeDetails(row) {
    ...
    Promise.all(promises).then((validations) => {
      ...
      const mutation = {
        firstName: updatedAttendee.editedAttendee.firstName,
        lastName: updatedAttendee.editedAttendee.lastName,
        email: updatedAttendee.editedAttendee.email,
        company: updatedAttendee.editedAttendee.company,
      };

      this.api.updateOrderAttendee(this.$route.params.eventId, updatedAttendee.orderId, updatedAttendee.id, mutation)
          .then((res) => {
