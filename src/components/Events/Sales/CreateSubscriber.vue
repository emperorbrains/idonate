  ...
  <b-modal :id="id" ref="modal" centered hide-footer hide-header>
    <div class="p-3">
      ...
      <div class="mb-4">
        ...
        <select-component
          name="subscriber-interval"
          :options="intervals"
          v-validate="'required'"
          :class="(errors.has('subscriber-interval'))? 'input-error' : ''"
          v-model="interval"
          @input="val => interval = val" />
      </div>
...
export default class CreateSubscriber extends BaseVue {
  ...
  private name: string = '';
  private intervals: any[] = EventConstants.SUBSCRIBER_INTERVALS;
  private interval: string = 'WEEKLY';
  ...
  private handleSubmit() {
    ...
    Promise.all(promises).then((validations) => {
      ...
      const mutation = {
        email: this.name,
        interval: this.interval || this.init.interval,
      };

      if (this.edit === true) {
        this.api.updateEmailSubscriber(this.$route.params.eventId, this.originalEmail, mutation)
        .then(() => {
            this.reset();
            this.$emit('refresh', mutation);
            this.$refs.modal.hide();
          }).catch(this.handleFailure);
      } else {
        this.api.addEmailSubscriber(this.$route.params.eventId, mutation)
          .then(() => {
            this.reset();
            this.$emit('refresh', mutation);
            this.$refs.modal.hide();
  ...
  private onEditChanged() {
    if (this.edit === true) {
      this.originalEmail = this.init.email;
