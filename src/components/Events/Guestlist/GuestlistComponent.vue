        ...
        <b-row class="mb-2 toggle-row toggle-row1-padding">
          ...
          <b-col md="5" class="d-flex">
            ...
            <input
              ...
              type="text"
              class="form-control form-control-sm rounded-pill"
              :class="{ 'border-danger text-danger': errors.has('guest-phone') }"
...
export default class GuestListComponent extends BaseEventVue {
  ...
  private rowClickHandler(item) {
    let guest = {};
    console.log('rowClickHandler', JSON.stringify(item));
    this.guestList.forEach((data: any, index) => {
      let isVisible = false;
      if (data.id === item.id) {
        isVisible = !TypeUtils.toBoolean(data._showDetails);
      }
  ...
  private saveData() {
    ...
    Promise.all(promises).then((validations) => {
      ...
      console.log(JSON.stringify(this.editGuest));
      const mutation = {
        firstName: this.editGuest.firstName,
        lastName: this.editGuest.lastName,
        email: this.editGuest.email,
        company: this.editGuest.company,
        salutation: this.editGuest.salutation,
        phone: this.editGuest.phone,
        // street: this.editGuest.street,
        // street2: this.editGuest.street2,
        // city: this.editGuest.city,
        // state: this.editGuest.state,
        // zip: this.editGuest.zip,
      };

      this.api.updateOrderAttendee(this.$route.params.eventId, this.editGuest.orderId, this.editGuest.id, mutation)
