<template>
  <div>
    ...
    <div v-show="croppieImage.length < 1">
      <div class="sponsor-image mb-3" v-if="sponsor.imageUrl && sponsor.imageUrl.length > 0">
        <img class="rounded" :src="sponsor.imageUrl" :alt="sponsor.name" >
...
@Component
export default class SponsorComponent extends BaseVue {
  public $refs!: {
    croppie: any;
  ...
  private deleteSponsor() {
    // this.$emit('delete-sponsor', this.sponsor.id);

    // if (ApiUtils.hasValue(this.sponsor.id)) {
    //   this.api.deleteSponsor(this.$route.params.eventId, this.sponsor.id)
    ...
    // TODO: Handle update of removed image.
  }
  ...
  private result(output) {
    const sponsor = _.cloneDeep(this.sponsor);
    sponsor.imageUrl = output;
    this.$emit('update-sponsor', sponsor);

    const mutation = {
      tierIndex: this.tierIndex,
      image: ImageUtils.dataURItoBlob(output),
      name: sponsor.name,
      webUrl: sponsor.link,
    };

    if (ApiUtils.hasValue(this.sponsor.id)) {
      this.api.updateSponsor(this.$route.params.eventId, this.sponsor.id, mutation)
        .then((res) => {
          this.sponsor.id = this.handleSuccess(res, 'event.createSponsor.sponsor.id', 'event.createSponsor.error');
          this.$emit('update-sponsor', sponsor);
        }).catch(this.handleFailure);
    } else {
      this.api.createSponsor(this.$route.params.eventId, mutation)
        .then((res) => {
          this.sponsor.id = this.handleSuccess(res, 'event.createSponsor.sponsor.id', 'event.createSponsor.error');
          this.$emit('update-sponsor', sponsor);
        }).catch(this.handleFailure);
