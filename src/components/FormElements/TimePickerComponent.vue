      ...
      <div class="row no-gutters time">
        ...
        <div class="col-sm px-1">
          <select class="form-control custom-select font-weight-semibold text-center" v-model="meridiem">
            <option value="AM" :disabled="disableMeridiem('AM')">AM</option>
            <option value="PM" :disabled="disableMeridiem('PM')">PM</option>
          </select>
        </div>
      </div>
...
export default class TimePickerComponent extends Vue {
  ...
  private disableMeridiem(meridiem) {
    const selectedTime = moment(this.value.dateTime);
    const today = moment();

    if (today.isBefore(selectedTime, 'date')) {
      return false;
    }

    if (selectedTime.hours() < 12 && meridiem === 'AM') {
      return false;
    } else if (selectedTime.hours() > 12 && meridiem === 'PM') {
      return false;
    }
